...
def get_full_db_schema():
    ...
    # (A) 테이블 정보
    sql_tables = """
    SELECT t.table_name, t.comments, c.column_name, c.data_type, cc.comments
    ...
    with get_oracle_conn() as conn:
        ...
        for row in cursor.fetchall():
            t_name, t_cmt, c_name, d_type, c_cmt = row
            if t_name not in table_map:
                t_desc = f" ({t_cmt})" if t_cmt else ""
                table_map[t_name] = {"header": f"Object: {t_name} (Type: Table){t_desc}", "cols": []}
        ...
        for t_name, data in table_map.items():
            ...
            documents.append({"content": content, "type": "table", "name": t_name})
        ...
        for key, lines in source_map.items():
            ...
            documents.append({"content": content, "type": "code", "name": name})
...
def get_all_table_names():
    with get_oracle_conn() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT table_name, comments FROM user_tab_comments ORDER BY table_name")
